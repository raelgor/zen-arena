...
export default async function make_session(user: User) {

   let sessions = await dataTransporter.get({
   ...
   await instance.cache.hmset(`sessions:${session.session_token}`, session);
