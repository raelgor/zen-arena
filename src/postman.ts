...
import increment_email from './fn/increment_email';
...
postman.email = async function(options) {
   ...
   if(!(await increment_email(options.to)))
      return Promise.resolve(false);

   return await postman._email.sendMail(options);
